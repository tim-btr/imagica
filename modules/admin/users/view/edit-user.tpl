<?php 
if(!isset($notice) ) { ?>
<table class="adm-comm-edit-table">
  <form action="" method="post">
    <tr>
       <td>Имя пользователя: <span>*</span></td>
	   <td><input type="text" name="name" value="<?php if(isset($_POST['name'])) {echo hc($_POST['name']);} else {echo hc($row['name']);} ?>"></td>
	   <td><?php if(isset($errors['name'])) {echo hc($errors['name']);} ?></td>
    </tr>
    <tr>
       <td>Логин:<span>*</span></td>
	   <td><input type="text" name="login" value="<?php if(isset($_POST['login'])){echo hc($_POST['login']);} else{echo hc($row['login']);}?>"></td>
	   <td><?php if(isset($errors['login'])) {echo hc($errors['login']);} ?></td>
    </tr>
    <tr>
       <td>Пароль:<span>*</span></td>
	   <td><input type="password" name="pass"></td>
	   <td><?php if(isset($errors['pass'])) {echo hc($errors['pass']);} ?></td>
    </tr>
    <tr>
       <td>Электронная почта<span>*</span></td>
	   <td><input type="text" name="email" value="<?php if(isset($_POST['email'])) {echo hc($_POST['email']);}else{echo hc($row['email']);} ?>"></td>
	   <td><?php if(isset($errors['email'])) {echo hc($errors['email']);} ?></td>
    </tr>
	<tr>
	  <td>Возраст</td>
	  <td><input type="text" name="age" value="<?php if(isset($row['age'])) {echo hc($row['age']);} ?>"></td>
	  <td><?php if(isset($errors['age'])) {echo hc($errors['age']);} ?></td>
	</tr>
	<tr>
	  <td>Пол пользователя</td>
	  <td>
			<label>
				<input type="radio" name="sex" value="f" align="left" <?php if(isset($row['sex']) && $row['sex'] == 'f') {echo 'checked';} ?>>Ж
			</label>
			<label>
				<input type="radio" name="sex" value="m" <?php if(!isset($row['sex']) || (isset($row['sex']) && $row['sex'] == 'm')) {echo 'checked';} ?>>М</label>
		</td>
	  <td></td>
	</tr>
	<tr>
		<?php
		if($row['role'] !== 'banned') { ?>
				<td>Внести пользователя в бан</td>
				<td><input type="checkbox" name="ban" value="ban"></td> <?php
		} else { ?>
			<td>Разбанить пользователя</td>
			<td><input type="checkbox" name="ban" value="unban"></td> <?php
		}
		?>
	</tr>
	
	<tr>
	  <td colspan="2" align="center"><input type="submit" value="Редактировать"></td>
	</tr>
	<tr>
  	  <td colspan="2" align="center"><span>*</span> - поле обязательно для заполнения</td>
  	</tr>
  </form>
</table>
<?php } else { 
	echo $notice;
}




	
	
	
	
